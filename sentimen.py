...
if not is_table_empty(table_name):
    ...
    file_path_knn = 'model/review.pkl'

    # Read the pickle file
    with open(file_path_knn, 'rb') as file:
        knn_model = pickle.load(file)
        
    # pembuatan vector kata
    vectorizer = TfidfVectorizer()
    train_vector = vectorizer.fit_transform(knn_model)
    reviews2 = [" ".join(r) for r in reviews]

    load_model = joblib.load(open('model/naive_bayes_ishihara.pkl', 'rb'))
